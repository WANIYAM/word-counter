...
async function mainMenu() {
    ...
    if (main.menu === "word counter") {
        ...
        let table = new Table();
        table.push({ "Word Count": countWords(userText.text) }, { "Character Count (with whitespace)": countCharactersWithWhitespace(userText.text) }, { "Character Count (without whitespace)": countCharactersWithoutWhitespace(userText.text) }, { "Unique Words Count": countUnique(userText.text).toString() } // Convert number to string 
        );
        // Display the table
...
function countCharactersWithWhitespace(text) {
    return text.length;
}
;
    ...
    let question = await inquirer.prompt([
        {
            ...
            choices: ["yes", "no"] // Removed unnecessary comma
        }
    ]);
    if (question.answer === "yes") {
        await mainMenu(); // Added "await" here
        await repeat(); // Added "await" here
    }
    else {
